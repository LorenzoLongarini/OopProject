...
public class MetadataArray {
	...
	public ArrayList<Metadata> metadataArrCreat(){
		...
		metadataArr.add(new Metadata("is_deleted", "If the file identified by the latest revision in the response is either deleted or moved.", "Boolean"));
		...
		metadataArr.add(new Metadata("client_modified", "For files, this is the modification time set by the desktop client when the file was added to Dropbox.", "Date"));
		...
		metadataArr.add(new Metadata("is_downloadable", "If true, file can be downloaded directly; else the file must be exported.", "Boolean"));
